...
public class TaskManagerService {
	 ...
	 */
	public boolean addProcess(UUID pid, int priority) {
	 ...
	 * @return True if the new process could be added, otherwise false
	 */
	public boolean addProcessFIFO(UUID pid, int priority) {
		MyProcess process = new MyProcess(pid, priority);
	 ...
	 * @return True if the new process could be added, otherwise false
	 */
	public boolean addProcessPriorityBased(UUID pid, int priority) {
		MyProcess process = new MyProcess(pid, priority);
		boolean success = blockingQueue.offer(process);
		if (!success) {
			...
			if (oldestProcess.getPriority() < priority) {
	...
	public Stream<MyProcess> killAll(int priority) {
		Iterator<MyProcess> iter = blockingQueue.iterator();
		while (iter.hasNext()) {
			MyProcess process = iter.next();
